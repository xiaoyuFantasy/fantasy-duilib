    ...
    case WM_MOUSEMOVE:
        {
			...
			{
				//拖拽
				LONG cx = abs(pt.x - m_ptLastMousePos.x);
				LONG cy = abs(pt.y - m_ptLastMousePos.y);
				CControlUI* pDragHover = FindControl(pt);
				if (pDragHover && m_bDragMode && (cx > 0 || cy > 0))
				{
					...
					//拖动前，让主窗口设置数据
					SendNotify(pDragHover, DUI_MSGTYPE_DRAG, (WPARAM)data_object);

					BITMAP bitmap_info;
					GetObject(m_hDragBitmap, sizeof(BITMAP), &bitmap_info);

					SIZE bitmap_size = { bitmap_info.bmWidth, bitmap_info.bmHeight };
					POINT ptDrag = { 0 };
					ptDrag.x = bitmap_info.bmWidth / 2;
					ptDrag.y = bitmap_info.bmHeight / 2;
					SdkDragSourceHelper dragSrcHelper;
					dragSrcHelper.InitializeFromBitmap(m_hDragBitmap, ptDrag, bitmap_size, data_object, RGB(255, 0, 255));
					...
					::PostMessage(m_hWndPaint, WM_LBUTTONUP, 0, 0);
					::PostMessage(m_hWndPaint, WM_SETCURSOR, 0, 0);
					return true;
				}
			}
			m_ptLastMousePos = pt;
...
void CPaintManagerUI::SetWindowAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if( _tcsicmp(pstrName, _T("size")) == 0 ) {
		...
		m_shadow.SetSize(_ttoi(pstrValue));
	}
	else if (_tcscmp(pstrName, _T("shadowsharpness")) == 0) {
		m_shadow.SetSharpness(_ttoi(pstrValue));
	}
	else if (_tcscmp(pstrName, _T("shadowdarkness")) == 0) {
		m_shadow.SetDarkness(_ttoi(pstrValue));
	}
		...
		m_shadow.SetImage(pstrValue);
	}
	else if (_tcscmp(pstrName, _T("showshadow")) == 0) {
		m_shadow.ShowShadow(_tcscmp(pstrValue, _T("true")) == 0);
	}
	else if (_tcscmp(pstrName, _T("drop")) == 0) {
		SetDropEnable(_tcscmp(pstrValue, _T("true")) == 0);
	}
    else 
