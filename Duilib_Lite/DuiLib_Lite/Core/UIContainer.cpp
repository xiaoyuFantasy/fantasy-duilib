...
namespace DuiLib_Lite
{
		...
		if (m_bAutoCalcWidth || m_bAutoCalcHeight)
		{
			...
			for (int it = 0; it < m_items.GetSize(); it++) {
				...
				if (pControl->IsFloat()) {
					...
					if (szXY.cx + szEstimate.cx > szMax.cx)
						szMax.cx = szXY.cx + szEstimate.cx;

					if (szXY.cy + szEstimate.cy > szMax.cy)
						szMax.cy = szXY.cy + szEstimate.cy;
				}
				else {
					SIZE szEstimate = pControl->EstimateSize(szAvailable);
					if (szEstimate.cx > szMax.cx)
			...
			szMax.cy += m_rcPadding.top + m_rcPadding.bottom;
			szMax.cx += m_rcInset.right + m_rcInset.left;
			szMax.cy += m_rcInset.bottom + m_rcInset.top;
			if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()) {
			...
			if (szAvailable.cx > szMax.cx)
				szMax.cx = szAvailable.cx;

			if (szAvailable.cy > szMax.cy)
				szMax.cy = szAvailable.cy;
	...
	void CContainerUI::SetAutoCalcWidth(bool bAutoCalc)
	{
		m_bAutoCalcWidth = bAutoCalc;
	}

	void CContainerUI::SetAutoCalcHeight(bool bAutoCalc)
	{
		m_bAutoCalcHeight = bAutoCalc;
	}
