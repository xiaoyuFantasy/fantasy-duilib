...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
	...
	m_ListInfo.bBottomVisible = false;
...
void CListUI::SetBottomVisible(bool bBottomVisible)
{
	m_ListInfo.bBottomVisible = bBottomVisible;
}

bool CListUI::IsBottomVisible()
{
	return m_ListInfo.bBottomVisible;
}
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("bottomvisible")) == 0) SetBottomVisible(_tcscmp(pstrValue, _T("true")) == 0);
...
void CListBodyUI::SetPos(RECT rc, bool bNeedInvalidate)
{
	...
	if (m_pOwner->IsBottomVisible() && m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible())
	{
		m_pOwner->SelectItem(m_pOwner->GetCount() - 1);
		m_pOwner->EnsureVisible(m_pOwner->GetCount() - 1);
	}
}
...
CListLabelElementUI::CListLabelElementUI() : m_bNeedEstimateSize(true), m_uFixedHeightLast(0),
    m_nFontLast(-1), m_uTextStyleLast(0), m_uTextStyle(DT_VCENTER | DT_SINGLELINE)
{
...
void CListLabelElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("valign")) == 0)
	{
		...
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		m_dwDisabledBkColor = clrColor;
	}
	else if (_tcscmp(pstrName, _T("flowtextrect")) == 0) {
		if (_tcscmp(pstrValue, _T("true")) == 0) m_bFlowTextRect = true;
	}
	else
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
{
	...
	if (m_bItemTextStyle) uTextStyle = m_uTextStyle;
    ...
    if (m_bNeedEstimateSize) {
        ...
        if ((uTextStyle & DT_SINGLELINE) != 0) {
            ...
            if (m_cxyFixedLast.cx == 0) {
                ...
                if( pInfo->bShowHtml ) {
                    int nLinks = 0;
                    CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, NULL, NULL, nLinks, pInfo->nFont, DT_CALCRECT | uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                }
                else {
                    CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, pInfo->nFont, DT_CALCRECT | uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                }

                m_cxyFixedLast.cx = rcText.right - rcText.left + pInfo->rcTextPadding.left + pInfo->rcTextPadding.right;

				m_cxyTextFixedLast.cx = rcText.right - rcText.left + pInfo->rcTextPadding.right + pInfo->rcTextPadding.left;
				m_cxyTextFixedLast.cy = m_cxyFixedLast.cy;
            }
        }
        else {
            if( m_cxyFixedLast.cx == 0 ) {
	...
	TFontInfo* fontInfo = m_pManager->GetFontInfo(pInfo->nFont);
	if (m_cxyFixedLast.cy > fontInfo->tm.tmHeight)
	{
		m_uTextStyle &= ~(DT_CENTER | DT_RIGHT);
		m_uTextStyle |= DT_LEFT;
	}
...
void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    if( pInfo->bShowHtml )
        ...
        NULL, NULL, nLinks, pInfo->nFont, uTextStyle);
    else
        CRenderEngine::DrawText(hDC, m_pManager, rcText, m_sText, iTextColor, \
        pInfo->nFont, uTextStyle);
}

void CListLabelElementUI::DrawItemBk(HDC hDC, const RECT & rcItem)
{
	ASSERT(m_pOwner);
	if (m_pOwner == NULL) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
	if (pInfo == NULL) return;
	DWORD iBackColor = 0;
	if (!pInfo->bAlternateBk || m_iDrawIndex % 2 == 0) iBackColor = m_dwBkColor == 0 ? pInfo->dwBkColor : m_dwBkColor;
	if ((m_uButtonState & UISTATE_HOT) != 0) {
		iBackColor = m_dwHotBkColor == 0 ? pInfo->dwHotBkColor : m_dwHotBkColor;
	}
	if (IsSelected()) {
		iBackColor = m_dwSelectedBkColor == 0 ? pInfo->dwSelectedBkColor : m_dwSelectedBkColor;
	}
	if (!IsEnabled()) {
		iBackColor = m_dwDisabledBkColor == 0 ? pInfo->dwDisabledBkColor : m_dwDisabledBkColor;
	}
	...
	if (iBackColor != 0) {
		...
		if (m_bFlowTextRect)
		{
			if ((m_uTextStyle & DT_RIGHT) != 0)
			{
				rcDraw.left = rcDraw.right - m_cxyTextFixedLast.cx;
			}
			else
			{
				rcDraw.right = rcDraw.left + m_cxyTextFixedLast.cx;
			}
		}
		CRenderEngine::DrawColor(hDC, rcDraw, GetAdjustColor(iBackColor));
	}
...
SIZE CListContainerElementUI::EstimateSize(SIZE szAvailable)
{
	...
	if (cXY.cy == 0) {
		...
		{
			CContainerUI *pCtrl = (CContainerUI*)GetItemAt(i);
			//pCtrl->SetAutoCalcHeight(true);
			SIZE szCtrl = pCtrl->EstimateSize(szAvailable);
			if (szCtrl.cy > maxHeight)
				maxHeight = szCtrl.cy;
		}
		cXY.cy = maxHeight;
	}
...
SIZE CListHBoxElementUI::EstimateSize(SIZE szAvailable)
{
	TListInfoUI* pInfo = NULL;
	if (m_pOwner) pInfo = m_pOwner->GetListInfo();
