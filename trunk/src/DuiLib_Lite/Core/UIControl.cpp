...
void CControlUI::SetDragEnabled(bool bDrag)
{
	m_bDragEnabled = bDrag;
}

bool CControlUI::IsDragEnabled()
{
...
void CControlUI::OnDragEnter(IDataObject * pDataObj, DWORD grfKeyState, POINT pt, DWORD * pdwEffect)
{
	if (IsDropEnabled())
	{
		*pdwEffect = DROPEFFECT_COPY;
		if (m_pManager != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_DROPENTER, (WPARAM)pDataObj);
	}
	else
	{
...
void CControlUI::OnDragLeave()
{
	if (IsDropEnabled())
	{
		if (m_pManager != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_DROPLEAVE);
	}
	else
...
void CControlUI::OnDrop(IDataObject * pDataObj, DWORD grfKeyState, POINT pt, DWORD * pdwEffect)
{
	...
	{
		*pdwEffect = DROPEFFECT_COPY;
		if (m_pManager != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_DROP, (WPARAM)pDataObj);
	}
	else
	{
		*pdwEffect = DROPEFFECT_NONE;
...
LPCTSTR CControlUI::GetCustomAttribute(LPCTSTR pstrName) const
{
	...
	if (!lpValue)
		return NULL;
	CDuiString* pCostomAttr = static_cast<CDuiString*>(lpValue);
	if( pCostomAttr ) 
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("drag")) == 0 ) SetDragEnabled(_tcscmp(pstrValue, _T("true")) == 0);
	else if (_tcscmp(pstrName, _T("drop")) == 0) SetDropEnabled(_tcscmp(pstrValue, _T("true")) == 0);
	else {
