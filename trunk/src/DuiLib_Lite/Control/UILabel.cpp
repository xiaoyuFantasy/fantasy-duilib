...
namespace DuiLib_Lite
{
	...
	SIZE CLabelUI::EstimateSize(SIZE szAvailable)
	{
		if (m_bAutoCalcWidth || m_bAutoCalcHeight)
		{
			...
			{
				if (m_cxyFixed.cy == 0) {
					m_cxyFixed.cy = m_pManager->GetFontInfo(m_iFont)->tm.tmHeight + 8;
					m_cxyFixed.cy += m_rcTextPadding.top + m_rcTextPadding.bottom;
				}

				if (m_bAutoCalcWidth)
				{
					RECT rcText = { 0, 0, 9999, m_cxyFixed.cy };
			...
			{
				if (m_bAutoCalcWidth)
					m_cxyFixed.cx = szAvailable.cx;

				if (m_bAutoCalcHeight)
				{
					RECT rcText = { 0, 0, m_cxyFixed.cx - (m_rcTextPadding.left + m_rcTextPadding.right), 9999 };
					...
					m_cxyFixed.cx = rcText.right - rcText.left + m_rcTextPadding.left + m_rcTextPadding.right;
					m_cxyFixed.cy = rcText.bottom - rcText.top + m_rcTextPadding.top + m_rcTextPadding.bottom;
				}
			}
			return m_cxyFixed;
		}
        ...
        if (m_bNeedEstimateSize) {
            ...
            if ((m_uTextStyle & DT_SINGLELINE) != 0) {
                ...
                if (m_cxyFixedLast.cx == 0) {
                    ...
                    m_cxyFixedLast.cx = rcText.right - rcText.left + m_rcTextPadding.left + m_rcTextPadding.right + m_rcPadding.left + m_rcPadding.right;
                }
            }
            else {
                if( m_cxyFixedLast.cx == 0 ) {
                ...
                m_cxyFixedLast.cy = rcText.bottom - rcText.top + m_rcTextPadding.top + m_rcTextPadding.bottom + m_rcPadding.top + m_rcPadding.bottom;
            }
        }
