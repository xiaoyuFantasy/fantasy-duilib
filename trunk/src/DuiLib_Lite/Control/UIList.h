...
public:
	...
	void SetBottomVisible(bool bBottomVisible);
...
public:
	...
	void SetPos(RECT rc, bool bNeedInvalidate = true)
	{
		...
		{
			CContainerUI *pLayout = (CContainerUI*)GetItemAt(i);
			CListHeaderItemUI* pHeaderItem = (CListHeaderItemUI*)pList->GetHeader()->GetItemAt(i);
			if (pLayout != nullptr && pHeaderItem != nullptr)
			{
				RECT rtHeader = pHeaderItem->GetPos();
				RECT rt = pLayout->GetPos();
				rt.left = rtHeader.left;
				rt.right = rtHeader.right;
				pLayout->SetPos(rt, bNeedInvalidate);
			}
		}
 	}
    void Invalidate(); // 直接CControl::Invalidate会导致滚动条刷新，重写减少刷新区域
