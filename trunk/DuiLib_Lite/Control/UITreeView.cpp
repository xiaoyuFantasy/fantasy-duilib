...
namespace DuiLib_Lite
{
	...
	// 函数说明: 
	//************************************
	void CTreeNodeUI::SetExpandTag(bool _IsExpand, bool bTriggerEvent)
	{
		m_bIsExpand = _IsExpand;
		SetExpandItem(_IsExpand);
		if (bTriggerEvent)
			m_pManager->SendNotify(this, DUI_MSGTYPE_TREEEXPAND, m_bIsExpand);
	}
	...
	// 函数说明: 
	//************************************
	bool CTreeNodeUI::GetExpandTag()
	{
		return m_bIsExpand;
	}

	void CTreeNodeUI::SetExpandItem(bool _Expanded)
	{
		if (this->GetCountChild() > 0)
		{
			int nCount = this->GetCountChild();
			for (int nIndex = 0; nIndex < nCount; nIndex++)
			{
				CTreeNodeUI* pItem = this->GetChildNode(nIndex);
				pItem->SetVisible(_Expanded);

				if (pItem->GetCountChild() && pItem->GetExpandTag()) //如果子项是展开的，那么子项的也展开
					pItem->SetExpandItem(_Expanded);
			}
		}
	...
	// 函数说明: 
	//************************************
	void CTreeNodeUI::RemoveAllNode()
	{
		if (mTreeNodes.GetSize() == 0)
			return;

		while (IsHasChild())
		{
			CTreeNodeUI* pNode = static_cast<CTreeNodeUI*>(mTreeNodes.GetAt(0));
			RemoveAt(pNode);
	...
	void CTreeNodeUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if (_tcscmp(pstrName, _T("text")) == 0)
			...
			SetExpandTag(_tcscmp(pstrValue, _T("true")) == 0, false);
	...
	CTreeNodeUI* CTreeNodeUI::CalLocation(CTreeNodeUI* _pTreeNodeUI)
	{
		...
		_pTreeNodeUI->SetVisible(GetExpandTag());
		return _pTreeNodeUI;
	}
	...
	/*****************************************************************************/
	/*****************************************************************************/
	void CTreeNodeUI::SetText(LPCTSTR pstrText)
	{
		pItemButton->SetText(pstrText);
	}

	CDuiString CTreeNodeUI::GetText() const
	{
		return pItemButton->GetText();
	}
	...
	bool CTreeViewUI::Add(CControlUI* pControl)
	{
		...
		// 去掉，取消TreeView控制folderbtn和checkbtn的显示，由treenode自己控制
	...
	long CTreeViewUI::AddAt(CTreeNodeUI* pControl, int iIndex)
	{
		...
		CTreeNodeUI* pParent = static_cast<CTreeNodeUI*>(GetItemAt(iIndex - 1)); // 添加 -1 应该取前一个Node的父节点
		if (!pParent) return -1;
	...
	bool CTreeViewUI::OnFolderChanged( void* param )
	{
		...
		if(pMsg->sType == DUI_MSGTYPE_SELECTCHANGED)
		{
			...
			pItem->SetExpandTag(!pItem->GetExpandTag());
			//pItem->SetExpandItem(pItem->GetExpandTag());
			//SetItemExpand(pItem->GetExpandTag(), pItem);
			/*if (pItem->GetExpandTag())
				m_pManager->SendNotify(pItem, DUI_MSGTYPE_TREEEXPAND);*/
			return true;
		}
		return true;
	}
	...
	bool CTreeViewUI::OnDBClickItem( void* param )
	{
		...
		if(pMsg->sType == DUI_MSGTYPE_ITEMDBCLICK)
		{
			...
			pFolder->Selected(!pFolder->IsSelected(), false);//改变节点的Folder状态
			pItem->SetExpandTag(!pItem->GetExpandTag()); //设置节点是否展开
			//pItem->SetExpandItem(pItem->GetExpandTag());
			//SetItemExpand(pItem->GetExpandTag(), pItem); //设置子节点
			//if (pItem->GetExpandTag())
			//	m_pManager->SendNotify(pItem, DUI_MSGTYPE_TREEEXPAND); //发送展开事件
			return true;
		}
		return false;
	}
