...
namespace DuiLib
{
	...
	{
		while (IsHasChild())
			RemoveAt(static_cast<CTreeNodeUI*>(mTreeNodes.GetAt(0)));
	...
	bool CTreeNodeUI::AddChildNode(CTreeNodeUI* _pTreeNodeUI)
	{
		...
		if (pTreeView) {
			...
			if (!pNode || !pNode->GetLastNode())
				nRet = pTreeView->AddAt(_pTreeNodeUI, GetTreeIndex() + 1) >= 0; //去掉 + 1
	...
	bool CTreeNodeUI::DoPaint(HDC hDC, const RECT & rcPaint, CControlUI * pStopControl)
	{
	...
	{
		DWORD iTextColor = m_dwItemTextColor;
	}
	...
	bool CTreeViewUI::OnFolderChanged(void* param)
	{
		...
		if (pMsg->sType == DUI_MSGTYPE_SELECTCHANGED)
		{
			...
			pItem->SetVisibleTag(pFolder->GetCheck()); //去掉!
			SetItemExpand(pFolder->GetCheck(),pItem); //去掉!
			//if (pItem)
			//{
			//	pItem->Expand(pFolder->GetCheck());
			//}
			return true;
		}
		return true;
